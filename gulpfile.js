...
async function copyFiles() {
	const config = fs.readJSONSync('./src/module.json');
	const statics = [
		'assets',
		'fonts',
		'lang',
...
async function linkUserData() {
	...
	try {
		...
		distModulePath = path.resolve('.', 'dist', 'module.json');
		sourceSystemPath = path.resolve('.', 'src', 'system.json');
		distSystemPath = path.resolve('.', 'dist', 'system.json');
		if (fs.existsSync(distModulePath) || fs.existsSync(sourceModulePath)) {
			destDir = 'modules';
			name = fs.readJSONSync(distModulePath).name;
		} else if (fs.existsSync(distSystemPath) || fs.existsSync(sourceSystemPath)) {
			destDir = 'systems';
			name = fs.readJSONSync(distSystemPath).name;
		} else {
			throw Error(
		...
		let linkDir;
		if (config.dataPath) {
			localAppDataPath = process.env.AppData.replace("Roaming", "Local");
